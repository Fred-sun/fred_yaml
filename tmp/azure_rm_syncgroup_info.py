...
DOCUMENTATION = '''
---
module: azure_rm_syncgroup_info
version_added: '2.9'
short_description: Get SyncGroup info.
description:
  - Get info of SyncGroup.
options:
  resource_group_name:
    description:
...
sync_groups:
  description: >-
    A list of dict results where the key is the name of the SyncGroup and the
    values are the facts for that SyncGroup.
  returned: always
  type: complex
  contains:
    value:
...
class AzureRMSyncGroupInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMSyncGroupInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['sync_groups'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.storage_sync_service_name is not None):
            self.results['sync_groups'] = self.format_item(self.listbystoragesyncservice())
        return self.results
    ...
    def listbystoragesyncservice(self):
        ...
        try:
            response = self.mgmt_client.sync_groups.list_by_storage_sync_service(resource_group_name=self.resource_group_name,
                                                                                 storage_sync_service_name=self.storage_sync_service_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
