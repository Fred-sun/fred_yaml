...
DOCUMENTATION = '''
---
module: azure_rm_globalschedule
version_added: '2.9'
short_description: Manage Azure GlobalSchedule instance.
description:
  - 'Create, update and delete instance of Azure GlobalSchedule.'
options:
  resource_group_name:
    description:
  ...
  state:
    description:
      - Assert the state of the GlobalSchedule.
      - >-
        Use C(present) to create or update an GlobalSchedule and C(absent) to
        delete it.
    default: present
    choices:
      - absent
...
class AzureRMGlobalSchedule(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.global_schedules.create_or_update(resource_group_name=self.resource_group_name,
                                                                          name=self.name,
                                                                          schedule=self.body)
            if isinstance(response, AzureOperationPoller) or isinstance(response, LROPoller):
                response = self.get_poller_result(response)
        except CloudError as exc:
            self.log('Error attempting to create the GlobalSchedule instance.')
            self.fail('Error creating the GlobalSchedule instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.global_schedules.delete(resource_group_name=self.resource_group_name,
                                                                name=self.name)
        except CloudError as e:
            self.log('Error attempting to delete the GlobalSchedule instance.')
            self.fail('Error deleting the GlobalSchedule instance: {0}'.format(str(e)))
