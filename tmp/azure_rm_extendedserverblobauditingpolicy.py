...
DOCUMENTATION = '''
---
module: azure_rm_extendedserverblobauditingpolicy
version_added: '2.9'
short_description: Manage Azure ExtendedServerBlobAuditingPolicy instance.
description:
  - >-
...
options:
  ...
  state:
    description:
      - Assert the state of the ExtendedServerBlobAuditingPolicy.
      - >-
        Use C(present) to create or update an ExtendedServerBlobAuditingPolicy
...
EXAMPLES = '''
    - name: Update a server's extended blob auditing policy with all parameters
      azure_rm_extendedserverblobauditingpolicy: 
...
class AzureRMExtendedServerBlobAuditingPolicy(AzureRMModuleBaseExt):
    def __init__(self):
        ...
        super(AzureRMExtendedServerBlobAuditingPolicy, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.extended_server_blob_auditing_policies.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the ExtendedServerBlobAuditingPolicy instance.')
            self.fail('Error creating the ExtendedServerBlobAuditingPolicy instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the ExtendedServerBlobAuditingPolicy instance.')
            self.fail('Error deleting the ExtendedServerBlobAuditingPolicy instance: {0}'.format(str(e)))

        return True

    def get_resource(self):
        try:
            response = self.mgmt_client.extended_server_blob_auditing_policies.get(resource_group_name=self.resource_group_name,
                                                                                   server_name=self.server_name,
                                                                                   blobauditingpolicyname=self.blobauditingpolicyname)
        except CloudError as e:
            return False
...
def main():
    AzureRMExtendedServerBlobAuditingPolicy()
