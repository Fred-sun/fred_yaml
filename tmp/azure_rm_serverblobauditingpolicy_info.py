...
DOCUMENTATION = '''
---
module: azure_rm_serverblobauditingpolicy_info
version_added: '2.9'
short_description: Get ServerBlobAuditingPolicy info.
description:
  - Get info of ServerBlobAuditingPolicy.
options:
  resource_group_name:
    description:
      - >-
...
EXAMPLES = '''
    - name: Get a server's blob auditing policy
      azure_rm_serverblobauditingpolicy_info: 
        resource_group_name: blobauditingtest-4799
        server_name: blobauditingtest-6440
        

    - name: List auditing settings of a server
      azure_rm_serverblobauditingpolicy_info: 
        resource_group_name: blobauditingtest-4799
        server_name: blobauditingtest-6440
...
server_blob_auditing_policies:
  description: >-
    ...
    ServerBlobAuditingPolicy and the values are the facts for that
...
class AzureRMServerBlobAuditingPolicyInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMServerBlobAuditingPolicyInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['server_blob_auditing_policies'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.server_name is not None):
            self.results['server_blob_auditing_policies'] = self.format_item(self.listbyserver())
        return self.results
    ...
    def listbyserver(self):
        ...
        try:
            response = self.mgmt_client.server_blob_auditing_policies.list_by_server(resource_group_name=self.resource_group_name,
                                                                                     server_name=self.server_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMServerBlobAuditingPolicyInfo()
