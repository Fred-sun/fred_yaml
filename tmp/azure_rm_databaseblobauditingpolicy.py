...
DOCUMENTATION = '''
---
module: azure_rm_databaseblobauditingpolicy
version_added: '2.9'
short_description: Manage Azure DatabaseBlobAuditingPolicy instance.
description:
  - 'Create, update and delete instance of Azure DatabaseBlobAuditingPolicy.'
options:
  resource_group_name:
    description:
      - >-
  ...
  state:
    description:
      - Assert the state of the DatabaseBlobAuditingPolicy.
      - >-
        Use C(present) to create or update an DatabaseBlobAuditingPolicy and
...
EXAMPLES = '''
    - name: Create or update a database's azure monitor auditing policy with minimal parameters
      azure_rm_databaseblobauditingpolicy: 
...
class AzureRMDatabaseBlobAuditingPolicy(AzureRMModuleBaseExt):
    def __init__(self):
        ...
        super(AzureRMDatabaseBlobAuditingPolicy, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.database_blob_auditing_policies.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the DatabaseBlobAuditingPolicy instance.')
            self.fail('Error creating the DatabaseBlobAuditingPolicy instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the DatabaseBlobAuditingPolicy instance.')
            self.fail('Error deleting the DatabaseBlobAuditingPolicy instance: {0}'.format(str(e)))
...
def main():
    AzureRMDatabaseBlobAuditingPolicy()
