...
DOCUMENTATION = '''
---
module: azure_rm_virtualmachineschedule
version_added: '2.9'
short_description: Manage Azure VirtualMachineSchedule instance.
description:
  - 'Create, update and delete instance of Azure VirtualMachineSchedule.'
options:
  resource_group_name:
    description:
  ...
  state:
    description:
      - Assert the state of the VirtualMachineSchedule.
      - >-
        Use C(present) to create or update an VirtualMachineSchedule and
...
class AzureRMVirtualMachineSchedule(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.virtual_machine_schedules.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the VirtualMachineSchedule instance.')
            self.fail('Error creating the VirtualMachineSchedule instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.virtual_machine_schedules.delete(resource_group_name=self.resource_group_name,
                                                                         lab_name=self.lab_name,
                                                                         virtual_machine_name=self.virtual_machine_name,
                                                                         name=self.name)
        except CloudError as e:
            self.log('Error attempting to delete the VirtualMachineSchedule instance.')
            self.fail('Error deleting the VirtualMachineSchedule instance: {0}'.format(str(e)))
