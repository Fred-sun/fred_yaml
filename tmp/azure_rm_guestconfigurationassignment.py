...
version_added: '2.9'
short_description: Manage Azure GuestConfigurationAssignment instance.
description:
  - 'Create, update and delete instance of Azure GuestConfigurationAssignment.'
options:
  ...
  state:
    description:
      - Assert the state of the GuestConfigurationAssignment.
      - >-
        Use C(present) to create or update an GuestConfigurationAssignment and
...
class AzureRMGuestConfigurationAssignment(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            ...
            self.log('Error attempting to create the GuestConfigurationAssignment instance.')
            self.fail('Error creating the GuestConfigurationAssignment instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.guest_configuration_assignments.delete(resource_group_name=self.resource_group_name,
                                                                               guest_configuration_assignment_name=self.guest_configuration_assignment_name,
                                                                               vm_name=self.vm_name)
        except CloudError as e:
            self.log('Error attempting to delete the GuestConfigurationAssignment instance.')
            self.fail('Error deleting the GuestConfigurationAssignment instance: {0}'.format(str(e)))
