...
DOCUMENTATION = '''
---
module: azure_rm_databaseaccountregion
version_added: '2.9'
...
class AzureRMDatabaseAccountRegion(AzureRMModuleBaseExt):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.database_account_region.create()
            else:
                response = self.mgmt_client.database_account_region.update()
    ...
    def delete_resource(self):
        try:
            response = self.mgmt_client.database_account_region.delete()
    ...
    def get_resource(self):
        try:
            response = self.mgmt_client.database_account_region.get()
        except CloudError as e:
            return False
        return response.as_dict()
