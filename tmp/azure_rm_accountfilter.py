...
DOCUMENTATION = '''
---
module: azure_rm_accountfilter
version_added: '2.9'
short_description: Manage Azure AccountFilter instance.
description:
  - 'Create, update and delete instance of Azure AccountFilter.'
options:
  resource_group_name:
    description:
  ...
  state:
    description:
      - Assert the state of the AccountFilter.
      - >-
        Use C(present) to create or update an AccountFilter and C(absent) to
        delete it.
    default: present
    choices:
      - absent
...
class AzureRMAccountFilter(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.account_filters.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the AccountFilter instance.')
            self.fail('Error creating the AccountFilter instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.account_filters.delete(resource_group_name=self.resource_group_name,
                                                               account_name=self.account_name,
                                                               filter_name=self.filter_name)
        except CloudError as e:
            self.log('Error attempting to delete the AccountFilter instance.')
            self.fail('Error deleting the AccountFilter instance: {0}'.format(str(e)))

        return True

    def get_resource(self):
        try:
            response = self.mgmt_client.account_filters.get(resource_group_name=self.resource_group_name,
                                                            account_name=self.account_name,
                                                            filter_name=self.filter_name)
        except CloudError as e:
            return False
