...
DOCUMENTATION = '''
---
module: azure_rm_jobexecution
version_added: '2.9'
short_description: Manage Azure JobExecution instance.
description:
  - 'Create, update and delete instance of Azure JobExecution.'
options:
  resource_group_name:
    description:
      - >-
  ...
  state:
    description:
      - Assert the state of the JobExecution.
      - >-
        Use C(present) to create or update an JobExecution and C(absent) to
        delete it.
    default: present
    choices:
      - absent
...
class AzureRMJobExecution(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.job_executions.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the JobExecution instance.')
            self.fail('Error creating the JobExecution instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the JobExecution instance.')
            self.fail('Error deleting the JobExecution instance: {0}'.format(str(e)))
