...
class AzureRMUserInfo(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['users'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.lab_account_name is not None and
              self.lab_name is not None):
            self.results['users'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.users.list(resource_group_name=self.resource_group_name,
                                                   lab_account_name=self.lab_account_name,
                                                   lab_name=self.lab_name,
                                                   expand=self.expand,
                                                   filter=self.filter,
