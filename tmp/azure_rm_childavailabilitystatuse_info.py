...
DOCUMENTATION = '''
---
module: azure_rm_childavailabilitystatuse_info
version_added: '2.9'
short_description: Get ChildAvailabilityStatuse info.
description:
  - Get info of ChildAvailabilityStatuse.
options:
  resource_uri:
    description:
      - >-
        The fully qualified ID of the resource, including the resource name and
        resource type. Currently the API only support one nesting level resource
        types :
...
EXAMPLES = '''
    - name: GetCurrentHealthByResource
      azure_rm_childavailabilitystatuse_info: 
...
child_availability_statuses:
  description: >-
    ...
    ChildAvailabilityStatuse and the values are the facts for that
    ChildAvailabilityStatuse.
  returned: always
  type: complex
  contains:
    id:
...
class AzureRMChildAvailabilityStatuseInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMChildAvailabilityStatuseInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_uri is not None):
            self.results['child_availability_statuses'] = self.format_item(self.getbyresource())
        elif (self.resource_uri is not None):
            self.results['child_availability_statuses'] = self.format_item(self.list())
        return self.results
    ...
    def getbyresource(self):
        ...
        try:
            response = self.mgmt_client.child_availability_statuses.get_by_resource(resource_uri=self.resource_uri,
                                                                                    filter=self.filter,
                                                                                    expand=self.expand)
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.child_availability_statuses.list(resource_uri=self.resource_uri,
                                                                         filter=self.filter,
                                                                         expand=self.expand)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMChildAvailabilityStatuseInfo()
