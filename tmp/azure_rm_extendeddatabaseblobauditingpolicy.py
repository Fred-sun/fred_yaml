...
DOCUMENTATION = '''
---
module: azure_rm_extendeddatabaseblobauditingpolicy
version_added: '2.9'
short_description: Manage Azure ExtendedDatabaseBlobAuditingPolicy instance.
description:
  - >-
...
options:
  ...
  state:
    description:
      - Assert the state of the ExtendedDatabaseBlobAuditingPolicy.
      - >-
        Use C(present) to create or update an ExtendedDatabaseBlobAuditingPolicy
...
EXAMPLES = '''
    - name: Create or update an extended database's azure monitor auditing policy with minimal parameters
      azure_rm_extendeddatabaseblobauditingpolicy: 
    ...
    - name: Create or update an extended database's blob auditing policy with minimal parameters
      azure_rm_extendeddatabaseblobauditingpolicy: 
        database_name: testdb
        resource_group_name: blobauditingtest-4799
        server_name: blobauditingtest-6440
...
class AzureRMExtendedDatabaseBlobAuditingPolicy(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            predicate_expression=dict(
                type='str',
                disposition='/predicate_expression'
            ),
            state=dict(
                type='sealed-choice',
                disposition='/state'
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.extended_database_blob_auditing_policies.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the ExtendedDatabaseBlobAuditingPolicy instance.')
            self.fail('Error creating the ExtendedDatabaseBlobAuditingPolicy instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the ExtendedDatabaseBlobAuditingPolicy instance.')
            self.fail('Error deleting the ExtendedDatabaseBlobAuditingPolicy instance: {0}'.format(str(e)))

        return True

    def get_resource(self):
        try:
            response = self.mgmt_client.extended_database_blob_auditing_policies.get(resource_group_name=self.resource_group_name,
                                                                                     server_name=self.server_name,
                                                                                     database_name=self.database_name,
                                                                                     blobauditingpolicyname=self.blobauditingpolicyname)
        except CloudError as e:
