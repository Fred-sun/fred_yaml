...
DOCUMENTATION = '''
---
module: azure_rm_jobtargetexecution_info
version_added: '2.9'
short_description: Get JobTargetExecution info.
description:
  - Get info of JobTargetExecution.
options:
  resource_group_name:
    description:
      - >-
...
EXAMPLES = '''
    - name: List job step target executions
      azure_rm_jobtargetexecution_info: 
...
job_target_executions:
  description: >-
    A list of dict results where the key is the name of the JobTargetExecution
    and the values are the facts for that JobTargetExecution.
  returned: always
  type: complex
  contains:
    value:
...
class AzureRMJobTargetExecutionInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMJobTargetExecutionInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.step_name is not None and
            self.target_id is not None):
            self.results['job_target_executions'] = self.format_item(self.get())
            ...
            self.results['job_target_executions'] = self.format_item(self.listbystep())
        elif (self.resource_group_name is not None and
              self.server_name is not None and
              self.job_agent_name is not None and
              self.job_name is not None and
              self.job_execution_id is not None):
            self.results['job_target_executions'] = self.format_item(self.listbyjobexecution())
        return self.results
    ...
    def get(self):
        ...
        try:
            response = self.mgmt_client.job_target_executions.get(resource_group_name=self.resource_group_name,
    ...
    def listbystep(self):
        ...
        try:
            response = self.mgmt_client.job_target_executions.list_by_step(resource_group_name=self.resource_group_name,
                                                                           server_name=self.server_name,
                                                                           job_agent_name=self.job_agent_name,
                                                                           job_name=self.job_name,
                                                                           job_execution_id=self.job_execution_id,
    ...
    def listbyjobexecution(self):
        ...
        try:
            response = self.mgmt_client.job_target_executions.list_by_job_execution(resource_group_name=self.resource_group_name,
                                                                                    server_name=self.server_name,
                                                                                    job_agent_name=self.job_agent_name,
                                                                                    job_name=self.job_name,
                                                                                    job_execution_id=self.job_execution_id,
