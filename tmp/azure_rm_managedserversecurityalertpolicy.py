...
DOCUMENTATION = '''
---
module: azure_rm_managedserversecurityalertpolicy
version_added: '2.9'
short_description: Manage Azure ManagedServerSecurityAlertPolicy instance.
description:
  - >-
...
options:
  ...
  state:
    description:
      - Assert the state of the ManagedServerSecurityAlertPolicy.
      - >-
        Use C(present) to create or update an ManagedServerSecurityAlertPolicy
...
EXAMPLES = '''
    - name: Update a server's threat detection policy with all parameters
      azure_rm_managedserversecurityalertpolicy: 
...
class AzureRMManagedServerSecurityAlertPolicy(AzureRMModuleBaseExt):
    def __init__(self):
        ...
        super(AzureRMManagedServerSecurityAlertPolicy, self).__init__(derived_arg_spec=self.module_arg_spec,
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.managed_server_security_alert_policies.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the ManagedServerSecurityAlertPolicy instance.')
            self.fail('Error creating the ManagedServerSecurityAlertPolicy instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.managed_server_security_alert_policies.delete()
        except CloudError as e:
            self.log('Error attempting to delete the ManagedServerSecurityAlertPolicy instance.')
            self.fail('Error deleting the ManagedServerSecurityAlertPolicy instance: {0}'.format(str(e)))
...
def main():
    AzureRMManagedServerSecurityAlertPolicy()
