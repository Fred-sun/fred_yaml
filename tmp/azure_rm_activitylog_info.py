...
DOCUMENTATION = '''
---
module: azure_rm_activitylog_info
version_added: '2.9'
short_description: Get ActivityLog info.
description:
  - Get info of ActivityLog.
options:
  filter:
    description:
      - >-
        Reduces the set of data collected.<br>This argument is required and it
        also requires at least the start date/time.<br>The **$filter** argument
        is very restricted and allows only the following patterns.<br>- *List
        events for a resource group*: $filter=eventTimestamp ge
        ...
        $filter=eventTimestamp ge '2014-07-16T04:36:37.6407898Z' and
        eventTimestamp le '2014-07-20T04:36:37.6407898Z'.<br>- *List events for
        a resource provider*: $filter=eventTimestamp ge
        '2014-07-16T04:36:37.6407898Z' and eventTimestamp le
...
activity_logs:
  description: >-
    A list of dict results where the key is the name of the ActivityLog and the
    values are the facts for that ActivityLog.
  returned: always
  type: complex
  contains:
    value:
...
class AzureRMActivityLogInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMActivityLogInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.filter is not None):
            self.results['activity_logs'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.activity_logs.list(filter=self.filter,
                                                           select=self.select)
