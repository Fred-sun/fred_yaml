...
DOCUMENTATION = '''
---
module: azure_rm_serversecurityalertpolicy_info
version_added: '2.9'
short_description: Get ServerSecurityAlertPolicy info.
description:
  - Get info of ServerSecurityAlertPolicy.
options:
  resource_group_name:
    description:
      - >-
...
EXAMPLES = '''
    - name: Get a server's threat detection policy
      azure_rm_serversecurityalertpolicy_info: 
...
server_security_alert_policies:
  description: >-
    ...
    ServerSecurityAlertPolicy and the values are the facts for that
...
class AzureRMServerSecurityAlertPolicyInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMServerSecurityAlertPolicyInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['server_security_alert_policies'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.server_name is not None):
            self.results['server_security_alert_policies'] = self.format_item(self.listbyserver())
        return self.results
    ...
    def listbyserver(self):
        ...
        try:
            response = self.mgmt_client.server_security_alert_policies.list_by_server(resource_group_name=self.resource_group_name,
                                                                                      server_name=self.server_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMServerSecurityAlertPolicyInfo()
