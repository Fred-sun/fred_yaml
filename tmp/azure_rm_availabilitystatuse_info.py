...
DOCUMENTATION = '''
---
module: azure_rm_availabilitystatuse_info
version_added: '2.9'
short_description: Get AvailabilityStatuse info.
description:
  - Get info of AvailabilityStatuse.
options:
  filter:
    description:
      - >-
...
EXAMPLES = '''
    - name: ListHealthBySubscriptionId
      azure_rm_availabilitystatuse_info: 
        {}
...
availability_statuses:
  description: >-
    A list of dict results where the key is the name of the AvailabilityStatuse
    and the values are the facts for that AvailabilityStatuse.
  returned: always
  type: complex
  contains:
    value:
    ...
    properties:
      ...
      contains:
        ...
        reason_type:
          description:
            - >-
              When the resource's availabilityState is Unavailable, it describes
              where the health impacting event was originated. Examples are
              planned, unplanned, user initiated or an outage etc.
          returned: always
          type: str
          sample: null
        root_cause_attribution_time:
          description:
            - >-
              When the resource's availabilityState is Unavailable, it provides
              the Timestamp for when the health impacting event was received.
          returned: always
          type: str
          sample: null
        health_event_type:
          description:
            - >-
              In case of an availability impacting event, it describes when the
              health impacting event was originated. Examples are Lifecycle,
        ...
        health_event_cause:
          description:
            - >-
              ...
              health impacting event was originated. Examples are
              PlatformInitiated, UserInitiated etc.
          returned: always
          type: str
          sample: null
        health_event_category:
          description:
            - >-
              In case of an availability impacting event, it describes the
              category of a PlatformInitiated health impacting event. Examples
        ...
        recommended_actions:
          ...
          contains:
            ...
            action_url_text:
              description:
                - >-
                  Substring of action, it describes which text should host the
                  action url.
              returned: always
              type: str
              sample: null
...
class AzureRMAvailabilityStatuseInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMAvailabilityStatuseInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None):
            self.results['availability_statuses'] = self.format_item(self.listbyresourcegroup())
        elif (self.resource_uri is not None):
            self.results['availability_statuses'] = self.format_item(self.getbyresource())
        elif (self.resource_uri is not None):
            self.results['availability_statuses'] = self.format_item(self.list())
        else:
            self.results['availability_statuses'] = self.format_item(self.listbysubscriptionid())
        return self.results
    ...
    def listbyresourcegroup(self):
        ...
        try:
            response = self.mgmt_client.availability_statuses.list_by_resource_group(resource_group_name=self.resource_group_name,
                                                                                     filter=self.filter,
                                                                                     expand=self.expand)
    ...
    def getbyresource(self):
        response = None

        try:
            response = self.mgmt_client.availability_statuses.get_by_resource(resource_uri=self.resource_uri,
                                                                              filter=self.filter,
                                                                              expand=self.expand)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.availability_statuses.list(resource_uri=self.resource_uri,
                                                                   filter=self.filter,
                                                                   expand=self.expand)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
    ...
    def listbysubscriptionid(self):
        ...
        try:
            response = self.mgmt_client.availability_statuses.list_by_subscription_id(filter=self.filter,
                                                                                      expand=self.expand)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMAvailabilityStatuseInfo()
