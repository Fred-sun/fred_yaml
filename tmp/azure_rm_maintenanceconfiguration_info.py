...
version_added: '2.9'
short_description: Get MaintenanceConfiguration info.
description:
  - Get info of MaintenanceConfiguration.
options:
  resource_group_name:
    description:
      - Resource Group Name
...
maintenance_configurations:
  description: >-
    ...
    MaintenanceConfiguration and the values are the facts for that
...
class AzureRMMaintenanceConfigurationInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMMaintenanceConfigurationInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['maintenance_configurations'] = self.format_item(self.get())
        else:
            self.results['maintenance_configurations'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.maintenance_configurations.list()
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
