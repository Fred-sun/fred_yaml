...
DOCUMENTATION = '''
---
module: azure_rm_serversecurityalertpolicy
version_added: '2.9'
short_description: Manage Azure ServerSecurityAlertPolicy instance.
description:
  - 'Create, update and delete instance of Azure ServerSecurityAlertPolicy.'
options:
  resource_group_name:
    description:
      - >-
  ...
  state:
    description:
      - Assert the state of the ServerSecurityAlertPolicy.
      - >-
        Use C(present) to create or update an ServerSecurityAlertPolicy and
...
EXAMPLES = '''
    - name: Update a server's threat detection policy with all parameters
      azure_rm_serversecurityalertpolicy: 
...
class AzureRMServerSecurityAlertPolicy(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            response = self.mgmt_client.server_security_alert_policies.create_or_update(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the ServerSecurityAlertPolicy instance.')
            self.fail('Error creating the ServerSecurityAlertPolicy instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the ServerSecurityAlertPolicy instance.')
            self.fail('Error deleting the ServerSecurityAlertPolicy instance: {0}'.format(str(e)))

        return True

    def get_resource(self):
        try:
            response = self.mgmt_client.server_security_alert_policies.get(resource_group_name=self.resource_group_name,
                                                                           server_name=self.server_name,
                                                                           security_alert_policy_name=self.security_alert_policy_name)
        except CloudError as e:
            return False
