...
DOCUMENTATION = '''
---
module: azure_rm_tdecertificate
version_added: '2.9'
short_description: Manage Azure TdeCertificate instance.
description:
  - 'Create, update and delete instance of Azure TdeCertificate.'
options:
  resource_group_name:
    description:
      - >-
  ...
  state:
    description:
      - Assert the state of the TdeCertificate.
      - >-
        Use C(present) to create or update an TdeCertificate and C(absent) to
        delete it.
    default: present
    choices:
      - absent
...
class AzureRMTdeCertificate(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.tde_certificates.create(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the TdeCertificate instance.')
            self.fail('Error creating the TdeCertificate instance: {0}'.format(str(exc)))
    ...
    def delete_resource(self):
        try:
            ...
            self.log('Error attempting to delete the TdeCertificate instance.')
            self.fail('Error deleting the TdeCertificate instance: {0}'.format(str(e)))
