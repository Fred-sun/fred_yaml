...
options:
  ...
  state:
    description:
      ...
      - >-
        Use C(present) to create or update an Replication and C(absent) to
        delete it.
    default: present
    choices:
      - absent
...
class AzureRMReplication(AzureRMModuleBaseExt):
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            region_endpoint_enabled=dict(
                type='bool',
                disposition='/region_endpoint_enabled'
            ),
            state=dict(
                type='str',
                default='present',
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.replications.create(resource_group_name=self.resource_group_name,
                                                                registry_name=self.registry_name,
                                                                replication_name=self.replication_name,
                                                                replication=self.body)
            else:
                response = self.mgmt_client.replications.update(resource_group_name=self.resource_group_name,
                                                                registry_name=self.registry_name,
                                                                replication_name=self.replication_name,
                                                                replication_update_parameters=self.body)
    ...
    def delete_resource(self):
        try:
            response = self.mgmt_client.replications.delete(resource_group_name=self.resource_group_name,
                                                            registry_name=self.registry_name,
                                                            replication_name=self.replication_name)
