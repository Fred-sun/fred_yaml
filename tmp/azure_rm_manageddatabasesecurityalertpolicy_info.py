...
DOCUMENTATION = '''
---
module: azure_rm_manageddatabasesecurityalertpolicy_info
version_added: '2.9'
short_description: Get ManagedDatabaseSecurityAlertPolicy info.
description:
  - Get info of ManagedDatabaseSecurityAlertPolicy.
options:
  resource_group_name:
    description:
      - >-
  ...
  database_name:
    description:
      - >-
        ...
        defined.
      - >-
        The name of the managed database for which the security alert policies
        are defined.
    required: true
    type: str
...
EXAMPLES = '''
    - name: Get a database's threat detection policy
      azure_rm_manageddatabasesecurityalertpolicy_info: 
        database_name: testdb
        managed_instance_name: securityalert-2080
        resource_group_name: securityalert-6852
        security_alert_policy_name: default
        

    - name: Get a list of the database's threat detection policies.
      azure_rm_manageddatabasesecurityalertpolicy_info: 
        database_name: testdb
        managed_instance_name: securityalert-2080
        resource_group_name: securityalert-6852
...
managed_database_security_alert_policies:
  description: >-
    ...
    ManagedDatabaseSecurityAlertPolicy and the values are the facts for that
...
class AzureRMManagedDatabaseSecurityAlertPolicyInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMManagedDatabaseSecurityAlertPolicyInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['managed_database_security_alert_policies'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.managed_instance_name is not None and
              self.database_name is not None):
            self.results['managed_database_security_alert_policies'] = self.format_item(self.listbydatabase())
        return self.results
    ...
    def listbydatabase(self):
        ...
        try:
            response = self.mgmt_client.managed_database_security_alert_policies.list_by_database(resource_group_name=self.resource_group_name,
                                                                                                  managed_instance_name=self.managed_instance_name,
                                                                                                  database_name=self.database_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMManagedDatabaseSecurityAlertPolicyInfo()
