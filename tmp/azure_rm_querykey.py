...
DOCUMENTATION = '''
---
module: azure_rm_querykey
version_added: '2.9'
short_description: Manage Azure QueryKey instance.
description:
  - 'Create, update and delete instance of Azure QueryKey.'
options:
  resource_group_name:
    description:
      - >-
  ...
  state:
    description:
      - Assert the state of the QueryKey.
      - >-
        Use C(present) to create or update an QueryKey and C(absent) to delete
        it.
    default: present
    choices:
      - absent
...
class AzureRMQueryKey(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.query_keys.create(resource_group_name=self.resource_group_name,
            ...
            self.log('Error attempting to create the QueryKey instance.')
            self.fail('Error creating the QueryKey instance: {0}'.format(str(exc)))
        return response.as_dict()

    def delete_resource(self):
        try:
            response = self.mgmt_client.query_keys.delete(resource_group_name=self.resource_group_name,
                                                          search_service_name=self.search_service_name,
                                                          key=self.key,
                                                          clientrequestid=self.clientrequestid,
                                                          parameters=self.body)
        except CloudError as e:
            self.log('Error attempting to delete the QueryKey instance.')
            self.fail('Error deleting the QueryKey instance: {0}'.format(str(e)))
