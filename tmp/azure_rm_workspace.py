...
options:
  state:
    description:
      ...
      - >-
        Use C(present) to create or update an Workspace and C(absent) to delete
        it.
    default: present
    choices:
      - absent
...
class AzureRMWorkspace(AzureRMModuleBaseExt):
    def __init__(self):
    ...
    def create_update_resource(self):
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.workspaces.create()
            else:
                response = self.mgmt_client.workspaces.update()
    ...
    def delete_resource(self):
        try:
            response = self.mgmt_client.workspaces.delete()
    ...
    def get_resource(self):
        try:
            response = self.mgmt_client.workspaces.get()
        except CloudError as e:
            return False
        return response.as_dict()
