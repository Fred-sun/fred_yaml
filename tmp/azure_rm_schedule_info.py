...
DOCUMENTATION = '''
---
module: azure_rm_schedule_info
version_added: '2.9'
short_description: Get Schedule info.
description:
  - Get info of Schedule.
options:
  resource_group_name:
    description:
...
schedules:
  description: >-
    A list of dict results where the key is the name of the Schedule and the
    values are the facts for that Schedule.
  returned: always
  type: complex
  contains:
    value:
...
class AzureRMScheduleInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMScheduleInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['schedules'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.lab_name is not None):
            self.results['schedules'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.schedules.list(resource_group_name=self.resource_group_name,
                                                       lab_name=self.lab_name,
                                                       expand=self.expand,
                                                       filter=self.filter,
                                                       top=self.top,
