...
DOCUMENTATION = '''
---
module: azure_rm_guestconfigurationhcrpassignmentreport_info
version_added: '2.9'
short_description: Get GuestConfigurationHCRPAssignmentReport info.
description:
  - Get info of GuestConfigurationHCRPAssignmentReport.
options:
  resource_group_name:
    description:
...
EXAMPLES = '''
    - name: List all guest configuration assignments for a virtual machine
      azure_rm_guestconfigurationhcrpassignmentreport_info: 
...
RETURN = '''
guest_configuration_hcrpassignment_reports:
  description: >-
    A list of dict results where the key is the name of the
    GuestConfigurationHCRPAssignmentReport and the values are the facts for that
...
class AzureRMGuestConfigurationHCRPAssignmentReportInfo(AzureRMModuleBase):
    def __init__(self):
        ...
        self.mgmt_client = None
        super(AzureRMGuestConfigurationHCRPAssignmentReportInfo, self).__init__(self.module_arg_spec, supports_tags=True)
    ...
    def exec_module(self, **kwargs):
        ...
        if (self.resource_group_name is not None and
            ...
            self.results['guest_configuration_hcrpassignment_reports'] = self.format_item(self.get())
        elif (self.resource_group_name is not None and
              self.guest_configuration_assignment_name is not None and
              self.machine_name is not None):
            self.results['guest_configuration_hcrpassignment_reports'] = self.format_item(self.list())
        return self.results
    ...
    def list(self):
        ...
        try:
            response = self.mgmt_client.guest_configuration_hcrpassignment_reports.list(resource_group_name=self.resource_group_name,
                                                                                        guest_configuration_assignment_name=self.guest_configuration_assignment_name,
                                                                                        machine_name=self.machine_name)
        except CloudError as e:
            self.log('Could not get info for @(Model.ModuleOperationNameUpper).')
...
def main():
    AzureRMGuestConfigurationHCRPAssignmentReportInfo()
